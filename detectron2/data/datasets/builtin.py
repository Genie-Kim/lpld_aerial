...
def register_all_dota(root):
    SPLITS = [
        ("dota_train", "dota_voc", "train"),
        ("dota_val", "dota_voc", "val"),
        ("dota_val_debug", "dota_voc", "valdebug"),
...
def register_all_gtav10k(root):
    SPLITS = [
        ...
        ("gtav10k_val", "gtav10k_voc", "val"),
    ]
    for name, dirname, split in SPLITS:
        year = 2007 if "2007" in name else 2012
        register_gtav10k(name, os.path.join(root, dirname), split, year)
        MetadataCatalog.get(name).evaluator_type = "gtav10k"
